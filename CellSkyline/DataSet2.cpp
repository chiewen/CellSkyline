...
DataSet2::DataSet2(int num, int layer): kDataPointNum(num), data_points(new vector<DataPoint2>()), layer(layer),
                             pt1(new bool[2][2][2]()),
                             pt2(new bool[4][4][4]()),
                             pt3(new bool[8][8][8]()),
...
void DataSet2::skyline_points(std::vector<DataPointD<2>>& points, std::vector<DataPointD<2>>& result) const
{
	sort(points.begin(), points.end(), [](const DataPoint2& p1, const DataPoint2& p2)-> bool
	{
		return p1[0] + p1[1] + p1[2] < p2[0] + p2[1] + p2[2];
	});

	vector<DataPoint2> skyline;
	for_each(points.begin(), points.end(), [&skyline](DataPoint2& point)-> void
	{
		if (!count_if(skyline.begin(), skyline.end(), [&point](DataPoint2& p_s) -> bool
		{
...
std::vector<DataPoint2> DataSet2::skyline_parallel()
{
	vector<DataPoint2> skyline;
	ParallelShrinker ps;
	auto cells = ps.shrink_parallel2(*this);
...
std::vector<DataPoint2> DataSet2::skyline_serial()
{
	vector<DataPoint2> skyline;


	vector<vector<KeyCell<2>>> kc(8);
	kc[0] = vector<KeyCell<2>>{ {0, 0, 0 } };
...
void DataSet2::init_data_points()
{
...
void DataSet2::sort_data_points(const int layer)
{

...
std::vector<Cell<D>> ParallelShrinker::process(std::vector<Cell<D>>& cells) const
{
	...
	thrust::device_vector<Cell<D>> d_cells_origin(d_cells);
	thrust::device_vector<Cell<D>> d_cells_result(cells.size());
	std::vector<Cell<D>> results(cells.size());

	thrust::inclusive_scan(d_cells.begin(), d_cells.end(), d_cells.begin(), CellComparer<D>());
	thrust::transform(d_cells.begin(), d_cells.end(), d_cells_origin.begin(), d_cells_result.begin(), Dominater<D>());

	thrust::copy(d_cells_result.begin(), d_cells_result.end(), results.begin());
	return results;
}
